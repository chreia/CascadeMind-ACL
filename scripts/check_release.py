...
def repository_files() -> list[str]:
    ...
    if proc.returncode != 0:
        raise RuntimeError(proc.stderr.strip() or "git ls-files --others failed")
    untracked = [line for line in proc.stdout.splitlines() if line]
...
def check_paper(errors: list[str]) -> None:
    ...
    required_snippets = [
        ...
        ("https://github.com/chreia/CascadeMind-ACL", "public code URL"),
...
def check_data(strict: bool, errors: list[str], warnings: list[str]) -> None:
    for rel, expected in EXPECTED_DATA_ROWS.items():
        path = ROOT / rel
        if not path.exists():
            message = f"{rel}: missing local data file"
            if strict:
